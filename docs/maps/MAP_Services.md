...
Key methods: `connect(provider:context:) async throws -> EmailAccount` (OAuth2+PKCE), `connectIMAP(email:password:host:provider:context:) async throws`, `disconnect(...)`, `runSyncPass(context:) async` / `startAutoSync(context:)` (15-min foreground loop), `static registerBackgroundSync/scheduleBackgroundRefresh` (BGTaskScheduler), `syncAll(accounts:context:) async`, `processEmail(_:accountId:context:) -> Bool` (classify→parse→learn→categorize→dedup→enqueue→maybe-auto-approve), `approveToLedger(item:context:autoApproved:)` (creates the permanent Transaction, updates Account balance + BNPL plan + learning services), `importPastedEmail/importSampleEmails`, `static recognizeAccount(bankName:cardLast4:currency:accounts:)`
