...
**Nothing is dropped silently**: when `BankSMSParser` returns no results the raw text is recorded to `UserDefaults` (`ft_sms_unparsed_v1`, newest 20, `UnparsedSMS` struct — no schema change) and listed under "Not recognized" in `SMSImportView`. An SMS that parses to nothing is either a non-transaction or a bank whose wording drifted; both need to be visible rather than vanishing, and it's the only way to see what text actually arrived from the Shortcuts automation.
