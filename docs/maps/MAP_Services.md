...
Key methods: `BankSMSParser.parse(rawText:senderId:userTemplates:) async -> [ParsedBankEmail]` (which tier produced a result is recorded only in `explanationLines`, not a separate return value — neither tier ever auto-approves), `BankSMSTemplateStore.active/identify(senderId:text:extraTemplates:)/refreshFromRemote(url:)` (opt-in remote JSON refresh — inert until a URL is actually configured, ships fully working offline on the bundled UAE-bank list), `FoundationModelsSMSExtractor.isAvailable/unavailableReason/extract(_:) async throws`
...
Purpose: orchestrates the whole email → pending-transaction pipeline — Gmail/Outlook OAuth2+PKCE, IMAP app-password, background sync, bank-email parsing, dedup, and ledger posting once the user approves in the review queue (no automated approval — see PROJECT_MAP §8).
...
Key methods: `connect(provider:context:) async throws -> EmailAccount` (OAuth2+PKCE), `connectIMAP(email:password:host:provider:context:) async throws`, `disconnect(...)`, `runSyncPass(context:) async` / `startAutoSync(context:)` (15-min foreground loop) / `stopAutoSync()`, `static registerBackgroundSync/scheduleBackgroundRefresh` (BGTaskScheduler; expiration handler calls `task.setTaskCompleted(success: false)` on cancel so iOS doesn't penalize future scheduling), `syncAll(accounts:context:) async`, `processEmail(_:accountId:context:) -> Bool` (classify→parse→learn→categorize→dedup→enqueue→`sendEmailImportAlert` unconditionally for every new item, with the current pending-review count for the notification badge — **no auto-approve step**, removed per user request; every item waits in the queue), `approveToLedger(item:context:autoApproved:)` (creates the permanent Transaction, updates Account balance + BNPL plan + learning services — only ever called from `EmailReviewQueueView`'s own approve tap now, `autoApproved` param kept but never passed `true`), `importPastedEmail/importSampleEmails`, `static recognizeAccount(bankName:cardLast4:currency:accounts:)`
...
Purpose: files a `BankSMSParser` result into the review queue — the SMS-side mirror of `EmailSyncService.processEmail`, reusing it and `.approveToLedger` directly rather than duplicating the pipeline. **No new `@Model` and no schema bump**: an SMS-sourced item is a normal `PendingEmailTransaction` whose `senderAddress` (and the matching `BankEmailRule.senderEmail`) carries an `"sms:<BankSMSTemplateStore.slug(bankName)>"` sentinel instead of an email address/domain — the same trick this codebase already uses for `AppSettings.cloudSyncEnabled`. `EmailSyncService.sync(account:context:)`'s email-fetch sender whitelist explicitly excludes `senderEmail.hasPrefix("sms:")` rules so they can't leak into a Gmail/IMAP/Outlook search. **No auto-approve**: every SMS-derived item lands `.pending` and waits for the user's own tap in `EmailReviewQueueView`, same as email (see PROJECT_MAP §8).
Singleton: no (`@MainActor enum`) | Actor: `@MainActor`
Key methods: `ingest(rawText:senderId:receivedAt:context:) async -> Bool` (drained from `RootView.drainPendingSMSTexts()`, called from the same foreground hooks as the Siri/Watch pending-intent queue), `senderTag(bankName:) -> String`, `unparsedMessages`/`clearUnparsed()`
**Nothing is dropped silently, and every delivery is logged**: each message reaching `ingest` is recorded to `UserDefaults` (`ft_sms_received_v1`, newest 20, `ReceivedSMS` struct — no schema change) with its outcome ("Added to the review queue" / "No transaction found" / duplicate), and listed under "Recent Messages" in `SMSImportView` alongside `WidgetDataService.pendingSMSCount` (undrained queue depth). Logging successes too is deliberate: an empty log proves the message never got past the queue, while an entry pins the problem to parsing — the difference that makes "the automation ran but nothing appeared" diagnosable. An SMS that parses to nothing is either a non-transaction or a bank whose wording drifted; both need to be visible rather than vanishing, and it's the only way to see what text actually arrived from the Shortcuts automation.
...
Singleton: `.shared` | Actor: implicit MainActor
Key methods: `updateAll(netWorth:currency:transactions:budgets:bills:payments:)` (+ `WidgetCenter.shared.reloadAllTimelines()`), `update(netWorth:currency:recentTransactions:)` (legacy/partial), `enqueuePendingTransaction(_:)`, `dequeuePendingTransactions() -> [PendingWidgetTransaction]` (drained by RootView), `enqueuePendingSMS(_:) -> Bool`/`dequeuePendingSMS() -> [PendingSMSText]`/`pendingSMSCount` (key `"pending_sms_texts"` — `LogTransactionFromText` enqueues raw SMS text here since an AppIntent can't reliably touch SwiftData; `RootView.drainPendingSMSTexts()` hands each to `SMSIngestService.ingest`), `enqueuePendingApplePay(_:) -> Bool`/`dequeuePendingApplePay()`/`pendingApplePayCount` (key `"pending_applepay"`). Both enqueues **read the value back after writing** and return whether it actually landed, so an intent can't report a success the store didn't give it; both queues resolve through `smsDefaults` (see the caveat below)
External APIs: WidgetKit, `UserDefaults(suiteName: "group.com.fintrack.shared")`
**App Group caveat**: the App Group entitlement needs a *paid* developer account and this project doesn't wire it (see PROJECT_MAP §8). Critically, `UserDefaults(suiteName:)` does **not** return nil without it — it returns a valid object (nil happens only for a nil name, the app's own bundle id, or the global domain) whose writes are never persisted, because the container directory doesn't exist. So a `?? .standard` fallback never fires and the failure is completely silent: a write "succeeds", the value survives only in that process's memory, and it's gone once the process exits. Widgets, Watch and `pending_transactions` genuinely need the entitlement and remain non-functional on device. The SMS and Apple Pay queues do **not** need it — the intents run in the app's own process (single application target, no App Intents Extension) — so `private var smsDefaults` is plain `.standard`, which always persists; `migrateLegacyQueue(forKey:)` moves anything an older build stranded in the group suite.
...
Singleton: `.shared` | Actor: implicit MainActor, `@Observable`
Key methods: `performBackup(context:) async -> Bool` (export → encrypt → write file → prune → refresh snapshot), `listBackups()`, `pruneOldBackups()` (`maxKeptBackups = 1` — only the newest backup is retained; also called from `LocalBackupView.reload()` to clear extras left by the old 10-file policy), `restore(from:context:mode:) async -> String`, `restoreFromDeviceSnapshotIfNeeded(container:) async -> Bool` (auto-restore at launch), `hasDeviceSnapshot`, `startObservingChanges(container:)`, `backupsDirectory` (also migrates legacy `Documents/Backups`).
