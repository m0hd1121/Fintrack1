...
**App Group caveat**: that suite returns **nil** unless the App Group entitlement is provisioned, which needs a *paid* developer account — this project doesn't wire it (see PROJECT_MAP §8), so every method here that guards on `UserDefaults(suiteName:)` is a silent no-op on device (widgets, Watch, `pending_transactions`). The SMS queue is the exception: it resolves through `private var smsDefaults` (shared suite if available, else `.standard`) because only the app's own process reads and writes it, so it works either way.
