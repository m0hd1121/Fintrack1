...
`DisableableFeature: String enum, 10 cases` (in `DisableableFeature.swift`) — 9 Settings "Premium Features" grid rows (`aiCFOMode`, `retirementSimulation`, `lifeEventPlanning`, `estatePlanning`, `insuranceOptimizer`, `smartCashAllocation`, `collaborativePlanner`, `financialEducation`, `remittanceTracker`) plus 1 standalone top-level section (`taxManagement`), disambiguated by `category: .premium | .topLevelSection` so `SettingsView` knows whether to filter it out of the Premium Features grid or hide its own `sectionCard`; each case has `symbol`/`tint` computed props mirroring the exact SF Symbol/`FTColor` used in `SettingsView.swift`; `static let disabledByDefault: Set<DisableableFeature>` currently `[.collaborativePlanner, .insuranceOptimizer, .remittanceTracker, .taxManagement]`. Toggled via `DisabledFeaturesView` ("Manage Features" row in Settings, lists all cases regardless of category); code/data for a disabled feature is never deleted, only hidden from the UI.
