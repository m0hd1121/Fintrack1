...
`DisableableFeature: String enum, 12 cases` (in `DisableableFeature.swift`) — 9 Settings "Premium Features" grid rows (`aiCFOMode`, `retirementSimulation`, `lifeEventPlanning`, `estatePlanning`, `insuranceOptimizer`, `smartCashAllocation`, `collaborativePlanner`, `financialEducation`, `remittanceTracker`), 2 standalone top-level sections (`taxManagement`, `businessFreelancer`), and 1 `.nested` case (`auditLog`, hidden inside `SecurityPrivacyView`'s `auditLogCard` rather than SettingsView), disambiguated by `category: .premium | .topLevelSection | .nested` — `.premium`/`.topLevelSection` are auto-filtered by `SettingsView`, `.nested` cases are checked manually via `isFeatureVisible(_:)` at their own call site (e.g. `SecurityPrivacyView.body` wraps `auditLogCard` in `if isFeatureVisible(.auditLog)`); each case has `symbol`/`tint` computed props mirroring the exact SF Symbol/`FTColor` used at its call site; `static let disabledByDefault: Set<DisableableFeature>` currently `[.collaborativePlanner, .insuranceOptimizer, .remittanceTracker, .taxManagement, .businessFreelancer, .auditLog]`. Toggled via `DisabledFeaturesView` ("Manage Features" row in Settings, lists all cases regardless of category); code/data for a disabled feature is never deleted, only hidden from the UI (audit **logging** itself keeps running in the background even while the UI is hidden — only the "Enable Audit Logging" toggle + "View Audit Log" screen are hidden).
