...
`DisableableFeature: String enum, 9 cases` (in `DisableableFeature.swift`) — one per Settings "Premium Features" row (`aiCFOMode`, `retirementSimulation`, `lifeEventPlanning`, `estatePlanning`, `insuranceOptimizer`, `smartCashAllocation`, `collaborativePlanner`, `financialEducation`, `remittanceTracker`), each with `symbol`/`tint` computed props mirroring the exact SF Symbol/`FTColor` used in `SettingsView.swift`; `static let disabledByDefault: Set<DisableableFeature>` currently `[.collaborativePlanner, .insuranceOptimizer, .remittanceTracker]`. Toggled via `DisabledFeaturesView` ("Manage Features" row in Settings); code/data for a disabled feature is never deleted, only hidden from the `NavigationLink` list.
