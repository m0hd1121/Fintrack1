...
## Bill.swift
`BillingCycle: String enum, 5 cases` · `BillCategory: String enum, 10 cases` (`.transactionCategory` maps each case to its closest `TransactionCategory`, e.g. `.subscriptions`→`.subscriptions`, `.healthcare`→`.medical`, `.housing`→`.rent` — used when `RecordPaymentSheet` creates a linked `Transaction`)
...
No fields added for SMS import: an SMS-sourced row is a normal `PendingEmailTransaction`/`BankEmailRule` whose `senderAddress`/`senderEmail` carries an `"sms:<slug>"` sentinel instead of an email address — see `SMSIngestService.swift` (MAP_Services.md).
...
`AuditEventType: String enum, 13 cases`
...
`AppSettings { id:UUID, useBiometrics:Bool, usePIN:Bool, pinHash:String?, autoLockMinutes:Int, showBalanceOnDashboard:Bool, defaultCurrency:String, twoFactorEnabled:Bool, twoFactorSecret:String?, auditLogEnabled:Bool, notificationsEnabled:Bool, budgetAlertsEnabled:Bool, billRemindersEnabled:Bool, salaryReminderEnabled:Bool, reminderDaysBefore:Int, lowBalanceAlertEnabled:Bool, lowBalanceThreshold:Double, largeTransactionAlertEnabled:Bool, largeTransactionThreshold:Double, goalMilestoneAlertEnabled:Bool, budgetAlertAt75:Bool, budgetAlertAt90:Bool, budgetAlertAt100:Bool, weeklyDigestEnabled:Bool, monthlyDigestEnabled:Bool, digestDayOfWeek:Int, digestDayOfMonth:Int, digestHour:Int, cloudSyncEnabled:Bool, backupWifiOnly:Bool, theme:AppTheme, accentColorName:String, oledMode:Bool, highContrastMode:Bool, fiscalYearStartMonth:Int, firstDayOfWeek:Int, dashboardHiddenWidgets:String }` — largest field count (34) of any @Model. `decoyPINHash`/`hiddenModeEnabled` fields were fully removed (Hidden Mode / Decoy PIN feature deleted outright per explicit user request, not just hidden — see §8). `encryptionEnabled` was also removed: encryption is now unconditionally forced on (no user toggle) — `SecurityPrivacyView.securityScore`/`scoreFeatureTile("Encrypted", ...)` hardcode it as always-active rather than reading a settings flag. `disabledFeatures` was also removed: disabling features is now **developer-only** (code + `docs/DISABLED_FEATURES.md`), no per-user persistence — see `DisableableFeature` below.
`AppTheme: String enum, 4 cases`
`DisableableFeature: String enum, 13 cases` (in `DisableableFeature.swift`) — 9 Settings "Premium Features" grid rows (`aiCFOMode`, `retirementSimulation`, `lifeEventPlanning`, `estatePlanning`, `insuranceOptimizer`, `smartCashAllocation`, `collaborativePlanner`, `financialEducation`, `remittanceTracker`), 2 standalone top-level sections (`taxManagement`, `businessFreelancer`), and 2 `.nested` cases (`auditLog`, hidden inside `SecurityPrivacyView`'s `auditLogCard`; `googleDriveBackup`, hidden as one sibling row inside `SettingsView`'s shared "Data & Privacy" `sectionCard`), disambiguated by `category: .premium | .topLevelSection | .nested` — `.premium`/`.topLevelSection` are auto-filtered by `SettingsView`, `.nested` cases are checked manually at their own call site (e.g. `SecurityPrivacyView.body` wraps `auditLogCard` in `if DisableableFeature.auditLog.isEnabled`; `SettingsView`'s Data & Privacy card wraps the Google Drive row+divider in `if DisableableFeature.googleDriveBackup.isEnabled`); each case has `symbol`/`tint` computed props mirroring the exact SF Symbol/`FTColor` used at its call site. **Developer-controlled only — no per-user storage, no in-app toggle screen.** The single source of truth is `static let disabled: Set<DisableableFeature>` (currently `[.collaborativePlanner, .insuranceOptimizer, .remittanceTracker, .taxManagement, .businessFreelancer, .auditLog, .googleDriveBackup]`), read via the pure computed `var isEnabled: Bool { !Self.disabled.contains(self) }`; kept in sync with `docs/DISABLED_FEATURES.md`. Code/data for a disabled feature is never deleted, only hidden (audit **logging** itself keeps running in the background even while the UI is hidden). **Exception**: Google Drive Backup's actual background sync (`RootView`'s `GoogleDriveBackupService.shared.syncIfDue`/`.startAutoSync` calls, 4 call sites) is also gated on `.googleDriveBackup.isEnabled` — unlike Audit Log, this feature actively transmits user data to a third party, so hiding its Settings entry point also stops the background sync loop, not just the UI.
...
- **`Transaction.amountInBaseCurrency` is the base value locked at the transaction's own creation-time rate** (historical), NOT a live figure. Set it when creating a `Transaction` via `CurrencyService.shared.amountInBase(amount, from: currency)`; then **display/aggregate it directly** (`$0 + tx.amountInBaseCurrency`) and never re-convert it with today's rate. Re-converting caused foreign amounts to drift and (in one case) double-count. If the init defaults it (no arg) it falls back to the raw `amount`, which is wrong for any foreign-currency transaction — always pass it explicitly unless the currency is guaranteed to be the base.
