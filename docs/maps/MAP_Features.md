...
### Features/AppIntents/
- FinTrackIntents.swift — Siri/Shortcuts intents: `LogExpenseIntent`, `LogIncomeIntent`, `LogTransactionFromText` (silent SMS→review-queue intent fed by a Shortcuts "When I receive a message" automation, see `SMSImportView`; **deliberately absent from `FinTrackShortcuts.appShortcuts`** — an App Shortcut is a fixed, parameterless entry point with no bindable fields, so registering it there made the automation picker offer an unconfigurable tile that prompted "enter a message" at every run), `LogApplePayTransaction` (Wallet fields from a Shortcuts "Transaction" automation; also deliberately not an App Shortcut). Its **`amount` is a `String`, not a `Double`** — Wallet hands Shortcuts a *formatted* amount and Shortcuts refuses to coerce it into a Number parameter, failing the whole automation with "couldn't convert from Text to Number"; the intent parses it with `TextNormalizer.decimalValue(in:)` and treats a negative as a refund. `date` stays a real `Date?` because Wallet supplies a genuine date — Shortcuts coerces **nothing** in either direction, so each parameter's type must match the bound variable exactly. It's optional and the walkthrough says to leave it unbound, since the automation fires at payment time and `date ?? Date()` is then accurate to the second, `GetBalanceIntent`, `GetBudgetStatusIntent`, `TransactionEntity`, `FinTrackShortcuts` provider
- FinTrackEntityIntents.swift — Siri/Spotlight entity layer: `AccountEntity`, `BudgetEntity`, `GoalEntity` (each with an `EntityStringQuery` so Siri resolves them by spoken name), `GetAccountBalanceIntent`, `GetGoalProgressIntent`, and the navigation intents `OpenFinTrackSectionIntent` (configurable) + `OpenTransactions`/`OpenBudget`/`OpenAccounts` (parameterless, registered as App Shortcuts). All read `IntentSnapshotSource`, never SwiftData — see PROJECT_MAP §8. `NavigationRequestStore` holds the requested section for `RootView.drainPendingNavigation()`.
...
- EmailReviewQueueView.swift — inbox-style swipe review (approve/reject/edit) of every automation-fed transaction, whatever channel it arrived on (source badge/label distinguishes email / SMS / Apple Pay). Shows **one row per real transaction**, not one per channel: `ImportDeduper` (MAP_Services.md) merges a payment that arrives on several channels into the single best-quality reading before it ever reaches this screen. A row still flagged `isPossibleDuplicate` (i.e. it matches something already **approved** into the ledger) shows an inline banner and needs an explicit “Approve Anyway” confirmation.
...
- ApplePayImportView.swift — Apple Pay import hub: Shortcuts "Transaction" automation walkthrough, "Recent Transactions" diagnostics (outcome + queue depth, same shape as SMS). Wallet delivers structured fields, so there's no parser and no per-bank config. See `ApplePayIngestService`/`ImportFiler` (MAP_Services.md).
- SMSImportView.swift (+ `SMSBankRuleSheet`) — SMS import hub: a single one-time Shortcuts setup walkthrough (`shortcuts://create-shortcut` deep link) — one automation with multiple senders selected under "From" covers every bank, since bank identification/parsing/account-matching are all automatic content-based (`BankSMSTemplateStore`/`EmailSyncService.recognizeAccount`) and need no app-side config. "My Banks (Optional)" (bank picker + SMS sender ID + linked account, reusing `BankEmailRule` tagged `"sms:…"`) exists only to pin a bank's transactions to a specific account or hint an unrecognized sender — no auto-approve control, every import waits for manual approval. First-SMS-within-24h status card, plus a "Recent Messages" section listing every SMS with its outcome in **three** distinct states (`outcomeSymbol`/`outcomeTint`): waiting (clock — Shortcuts delivered it, the app hasn't drained it), added/merged (check), or received-but-unparsed (warning), from `SMSIngestService.receivedMessages`, with raw text, time and Clear, plus the undrained queue depth — so a message never vanishes silently and you can see exactly what the Shortcuts automation delivered. See `BankSMSParser`/`SMSIngestService` (MAP_Services.md).

**Core features:** email-based bank ingestion (`EmailImportView` + `BankSetupWizardView` + `EmailReviewQueueView`, driven by `EmailSyncService`); SMS-based bank ingestion (`SMSImportView`, driven by `BankSMSParser`/`SMSIngestService`); Apple Pay ingestion (`ApplePayImportView`, driven by `ApplePayIngestService`) — all three file through `ImportFiler`/`ImportDeduper` into the same `EmailReviewQueueView`, collapsing multi-channel copies of one payment into a single best-quality row; file-based import (`OFXImportView`, `PDFImportView`, `ImportIntegrationView`); 3 backup/restore providers (`LocalBackupView` (offline/on-device), `EmailBackupView`, `GoogleDriveBackupView`), each wrapping its own `*BackupService.shared`.
