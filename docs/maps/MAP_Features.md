...
- SMSImportView.swift (+ `SMSBankRuleSheet`) — SMS import hub: Shortcuts setup walkthrough (`shortcuts://create-shortcut` deep link), per-bank config (bank picker + SMS sender ID + linked account, reusing `BankEmailRule` tagged `"sms:…"` — no auto-approve control, every import waits for manual approval), first-SMS-within-24h status card. See `BankSMSParser`/`SMSIngestService` (MAP_Services.md).
