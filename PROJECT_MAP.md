...
- **Hiding a feature/module must never delete its code, model, or data — use the `DisableableFeature` system.** `AppSettings.disabledFeatures: String?` (comma-separated raw values, `nil` = `DisableableFeature.disabledByDefault`) is read/written only through `AppSettings.disabledFeatureSet`/`.isFeatureEnabled(_:)` in `DisableableFeature.swift`, mirroring the pre-existing `dashboardHiddenWidgets` comma-separated-`String` convention rather than inventing a new storage mechanism. Each case has a `category`: `.premium` rows are filtered out of `SettingsView`'s "Premium Features" grid via `visibleFeatures` + `destinationView(for:)` switch; `.topLevelSection` cases (e.g. `.taxManagement`) instead wrap their own standalone `sectionCard` in `if isFeatureVisible(_:)`. Settings always shows a "Manage Features" row → `DisabledFeaturesView` (toggle list over *all* cases regardless of category, no own `NavigationStack` since it's pushed). **Any future "disable feature X" request should add a case to `DisableableFeature` (picking the right `category`) and nothing else** — never comment out or delete the feature's `NavigationLink`/`sectionCard`/view/model.
