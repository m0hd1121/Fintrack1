Last verified: 2026-07-20 @ 48b382c
...
- **Local data encryption is mandatory and not user-configurable — no toggle, no algorithm name in any user-facing text.** `AppSettings.encryptionEnabled` was removed entirely (per explicit user request to make encryption "forced"); `SecurityPrivacyView`'s "End-to-End Encryption" row is a static, non-interactive `securityRow` reading "Always on", and `securityScore`/`scoreFeatureTile("Encrypted", ...)` hardcode it as always-active. Separately, "AES-256" wording was scrubbed from all three user-facing strings that named the algorithm (`SecurityPrivacyView.infoCard`, `BackupEncryptionSettingsView`, `iCloudSyncView`) — replaced with generic "encrypted"/"strong encryption" phrasing. The `BackupEncryptionService` (passphrase-based export/iCloud/Drive backup encryption, a separate opt-in feature) was intentionally left as user-toggleable — the "force it" request was read as covering general local-data encryption, not the passphrase-backup workflow. Any new encryption-related UI must follow both rules: no on/off control for core data encryption, and never name the algorithm/mode to the user (code comments are fine, e.g. `BackupEncryptionSettingsView.swift`'s header comment still says AES-256-GCM).
...
- **Hiding a feature/module must never delete its code, model, or data — use the `DisableableFeature` system.** `AppSettings.disabledFeatures: String?` (comma-separated raw values, `nil` = `DisableableFeature.disabledByDefault`) is read/written only through `AppSettings.disabledFeatureSet`/`.isFeatureEnabled(_:)` in `DisableableFeature.swift`, mirroring the pre-existing `dashboardHiddenWidgets` comma-separated-`String` convention rather than inventing a new storage mechanism. Each case has a `category`: `.premium` rows are filtered out of `SettingsView`'s "Premium Features" grid via `visibleFeatures` + `destinationView(for:)` switch; `.topLevelSection` cases (e.g. `.taxManagement`) wrap their own standalone `sectionCard` in `if isFeatureVisible(_:)`; `.nested` cases live inside some other screen entirely (e.g. `.auditLog` inside `SecurityPrivacyView`'s `auditLogCard`) and that screen defines its own local `isFeatureVisible(_:)` mirroring `SettingsView`'s (same one-line nil-fallback logic, duplicated per-file by design — matches this codebase's existing small-helper-duplication style, e.g. `settingsBind`). Settings always shows a "Manage Features" row → `DisabledFeaturesView` (toggle list over *all* cases regardless of category, no own `NavigationStack` since it's pushed). Hiding UI is independent of the underlying mechanism continuing to run — e.g. hiding Audit Log's UI does not stop `AuditLogService.log(...)` calls elsewhere from still writing `AuditLogEntry` rows. **Any future "disable/hide feature X" request should add a case to `DisableableFeature` (picking the right `category`) and nothing else** — never comment out or delete the feature's `NavigationLink`/`sectionCard`/view/model, and never rip out the code path that produces its underlying data. **Exception**: if the user explicitly says "remove"/"delete" rather than "disable"/"hide", confirm which they mean before acting (this codebase has treated the two very differently: every "disable X" this session got the non-destructive `DisableableFeature` treatment, but "remove Hidden Mode / Decoy PIN" — after an explicit confirmation prompt — got a full deletion: `HiddenModeView`, `PINSetupSheet.isDecoy`, `AppSettings.decoyPINHash`/`.hiddenModeEnabled`, `AppState.isHiddenMode`, and `AuditEventType.hiddenModeActivated` no longer exist in the codebase at all, not just hidden).
...
- Audit Log UI hidden by default: new `.nested`-category `DisableableFeature` case (`.auditLog`) added to `disabledByDefault` (now `[.collaborativePlanner, .insuranceOptimizer, .remittanceTracker, .taxManagement, .businessFreelancer, .auditLog]`); `SecurityPrivacyView.auditLogCard` (the "Enable Audit Logging" toggle + "View Audit Log" screen) is now conditionally rendered via its own local `isFeatureVisible(_:)` — background audit logging (`AuditLogService.log`) is unaffected and keeps writing `AuditLogEntry` rows regardless of UI visibility.
- Business & Freelancer added as a second `.topLevelSection` `DisableableFeature` case and to `disabledByDefault` — hidden out of the box, code/data untouched, re-enable via Settings > Manage Features.
