Last verified: 2026-07-08 @ f504ec436189cbf48332a54e3b9be329ca40f12d
...
- **Currency correctness**: any code that sums or compares two currency-tagged fields (`Budget.amount`, `SavingsGoal.targetAmount`, `Account.balance`, etc.) across records must call `CurrencyService.convert(_:from:to:)` first. Grep for raw `.reduce(0) { $0 + $1.amount }`-style sums on currency-tagged fields as a red flag — several real bugs of this shape were found and fixed in Budget/SavingsGoal totals this session (`BudgetView.swift`, `ReportsView.SavingsGoalsReport`, `AddSavingsGoalView`'s emergency-fund suggestion, `SmartCashAllocationView`'s goal-funding calc). **Not yet fixed**: `SmartCashAllocationView.totalCash` (sums `Account.balance` across accounts, no conversion) and the `BudgetEnvelope` currency path (dormant — no picker sets a non-base currency yet, see STATE).
...
- **`.swipeActions` silently does nothing outside a `List`.** Several feature screens (`BudgetView.swift`'s tabs, `DebtManagementView.swift`'s tabs) use `ScrollView` + `VStack` instead of `List` for custom row styling, and `.swipeActions` on a row in that context never renders — no crash, no warning, just a dead no-op. Every row that needs swipe-style actions in a non-`List` container must use `.contextMenu` (long-press) instead; this is the established working pattern across Loans/BNPL/Lent/Borrowed/Budget/Goals/Envelopes. When adding a new delete/edit/archive action to a row, check the enclosing container before reaching for `.swipeActions`.
...
- `BudgetEnvelope.currency` exists on the model but `AddEnvelopeView` has no currency picker (always base currency) — the envelope-side cross-currency bug is dormant/unreachable until a picker is added; if one is added, `EnvelopeRow`/`envelopeOverviewCard` in `BudgetView.swift` need the same convert-before-sum fix already applied to Budget/SavingsGoal.
