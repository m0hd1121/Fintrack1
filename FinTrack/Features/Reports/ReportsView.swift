...
struct ChequesReport: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            if cheques.isEmpty {
                EmptyStateView(icon: "signature", title: "No Cheques", message: "Cheque payments you add will show up here as their date approaches.")
            } else {
                HStack(spacing: 0) {
                    summaryItem(label: "Total Due", value: totalDue.formatted(as: currency), color: .primary)
                    Divider().frame(height: 36)
                    summaryItem(label: "Cheques", value: "\(cheques.count)", color: .secondary)
                    Divider().frame(height: 36)
                    summaryItem(label: "Overdue", value: "\(overdueCount)", color: overdueCount > 0 ? FTColor.expense : .secondary)
                }
    ...
    private func summaryItem(label: String, value: String, color: Color) -> some View {
        VStack(spacing: 3) {
            Text(value).font(.ftHeadline).foregroundStyle(color)
    ...
    private func groupSection(_ group: (label: String, cheques: [Transaction])) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text(group.label)
                    .font(.ftCallout)
                    .foregroundStyle(group.label == "Overdue" ? FTColor.expense : .secondary)
                    .textCase(.uppercase)
                    .tracking(0.5)
                Spacer()
                let groupTotal = group.cheques.reduce(0) { $0 + currencyService.convert($1.amount, from: $1.currency, to: currency) }
                Text(groupTotal.formatted(as: currency))
                    .font(.ftCaption)
        ...
        return HStack(spacing: 12) {
            ZStack {
                ...
                Image(systemName: "signature")
                    .foregroundStyle(overdue ? FTColor.expense : FTColor.gold)
                    .font(.ftHeadline)
            }

            VStack(alignment: .leading, spacing: 2) {
                Text(tx.title).font(.ftBody)
            ...
            VStack(alignment: .trailing, spacing: 2) {
                Text(tx.amount.formatted(as: tx.currency))
                    .font(.ftBodySemibold)
                    .foregroundStyle(overdue ? FTColor.expense : .primary)
                if let chequeDate = tx.chequeDate {
                    Text(dueLabel(for: chequeDate, overdue: overdue))
                        .font(.caption)
                        .fontWeight(.medium)
    ...
    private func dueLabel(for date: Date, overdue: Bool) -> String {
        if overdue {
    ...
    private func urgencyColor(for date: Date, overdue: Bool) -> Color {
        if overdue { return FTColor.expense }
        let days = Calendar.current.dateComponents([.day],
            from: Calendar.current.startOfDay(for: Date()),
            to: Calendar.current.startOfDay(for: date)).day ?? 0
        return days <= 3 ? FTColor.expense : days <= 7 ? .orange : .secondary
    }
}
