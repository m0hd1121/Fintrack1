...
struct EstatePlanningView: View {
    ...
    private var assetBreakdownItems: [(String, String, Double, Color)] {
        [
            ("Cash & Accounts", "dollarsign.circle.fill", accounts.reduce(0) { $0 + $1.balance }, FTColor.accent),
            ("Real Estate", "house.fill", realEstateProps.reduce(0) { $0 + $1.currentValue }, FTColor.catTeal),
            ("Vehicles", "car.fill", vehicles.reduce(0) { $0 + $1.currentValue }, FTColor.catBlue),
            ("Investments", "chart.line.uptrend.xyaxis", investments.reduce(0) { $0 + $1.currentValue }, FTColor.income),
            ("Crypto", "bitcoinsign.circle.fill", cryptoHoldings.reduce(0) { $0 + $1.currentValue }, FTColor.catCoral),
            ("Gold", "circle.fill", goldHoldings.reduce(0) { $0 + $1.currentValue }, FTColor.gold),
            ("Personal Assets", "bag.fill", personalAssets.reduce(0) { $0 + $1.estimatedMarketValue }, FTColor.catPurple),
        ].filter { $0.2 > 0 }
    }
